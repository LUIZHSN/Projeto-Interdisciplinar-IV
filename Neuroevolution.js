var Neuroevolution = function (options) {
	...
	self.options = {
		...
		// vários fatores e parâmetros (junto com valores padrão).
		network:[2, [2], 1], // Estrutura de rede Perceptron (1 camada // oculta).
		population: 50, // População por geração.
		elitism: 0.2, // Melhores redes mantém-se inalteradas para o próximo
		// geração (taxa).
		randomBehaviour: 0.2, // novas redes aleatórias para a próxima geração
		// (avaliar).
		mutationRate: 0.1, // Taxa de mutação nos pesos das sinapses.
		mutationRange: 0.5, // O intervalo da mutação muda no
		// peso da sinapse.
		historic: 0, // Últimas gerações salvas.
		lowHistoric: false, // Apenas salve a pontuação (não a rede).
		scoreSort: -1, // Ordem de classificação (-1 = desc, 1 = asc).
		nbChild: 1 // Número de filhos por reprodução.
	}
	...
	Network.prototype.compute = function (inputs) {
		for (var i in inputs) {
			if (this.layers[0] && this.layers[0].neurons[i]) {
				this.layers[0].neurons[i].valor = inputs[i];
			}
		}
		...
		for (var i = 1; i < this.layers.length; i++) {
			for (var j in this.layers[i].neurons) {
				...
				for (var k in prevLayer.neurons) {
					sum += prevLayer.neurons[k].valor *
						this.layers[i].neurons[j].weights[k];
				}
				this.layers[i].neurons[j].valor = self.options.activation(sum);
			}
		...
		for (var i in lastLayer.neurons) {
			out.push(lastLayer.neurons[i].valor);
		}
		return out;
	}
